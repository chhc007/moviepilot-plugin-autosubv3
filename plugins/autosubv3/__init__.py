...
class AutoSubv3(AutoSubv3CompatMixin, _PluginBase):
    ...
    def init_plugin(self, config=None):
        ...
        # Hy-MT2 专用翻译方案开关（v3.6.0 定制，移植自 AutoSubv2 v3.0.x）
        self._use_hymt2 = config.get('use_hymt2', False)
        self._hymt2_translator = None
        if self._translate_zh and self._use_hymt2:
            # Hy-MT2 方案：独立配置，不依赖 AI API 端点池
            hy_key = config.get('hy_mt2_key')
            if not hy_key:
                logger.error("Hy-MT2翻译方案缺少API密钥配置，请填写 hy_mt2_key")
            else:
                try:
                    from .translate.hymt2_translate import Hymt2Translator
                except Exception as e:
                    logger.error(f"Hy-MT2翻译模块加载失败：{e}")
                else:
                    self._hymt2_translator = Hymt2Translator(
                        api_key=hy_key,
                        api_url=config.get('hy_mt2_url', "http://192.168.123.146:11436"),
                        model=config.get('hy_mt2_model', "hymt2-q8"),
                        batch_size=config.get('hy_mt2_batch_size', 20),
                        context_window=config.get('hy_mt2_context_window', 10),
                        max_retries=config.get('hy_mt2_max_retries', 3),
                        fallback=config.get('hy_mt2_fallback', True),
                        logger=logger,
                    )
                    logger.info(
                        "[AutoSubv3] Hy-MT2 专用翻译方案已启用 model=%s url=%s batch=%s ctx=%s retries=%s fallback=%s",
                        self._hymt2_translator._model,
                        config.get('hy_mt2_url', "http://192.168.123.146:11436"),
                        self._hymt2_translator._batch_size,
                        self._hymt2_translator._context_window,
                        self._hymt2_translator._max_retries,
                        self._hymt2_translator._fallback,
                    )
        ...
        if self._translate_zh:
            ...
            if not configured_endpoints and not self._use_hymt2:
            ...
            if configured_endpoints:
                ...
                logger.info("[AutoSubv3] 已启用 Hy-MT2 专用翻译方案，未加载 AI API 端点池（Hy-MT2 走独立客户端）")
