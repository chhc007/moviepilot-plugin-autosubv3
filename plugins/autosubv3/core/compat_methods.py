...
class AutoSubv3CompatMixin:
    ...
    def _AutoSubv3__translate_zh_subtitle(self, source_lang: str, source_subtitle: str, dest_subtitle: str,
        ...
        """
        # Hy-MT2 专用翻译方案（v3.6.0 定制）：独立旁路，绕过 v3 端点池/双语管线，
        # 输出仅中文译文替换原字幕 content，条目数与时间轴保持不变
        if getattr(self, '_use_hymt2', False):
            translator = getattr(self, '_hymt2_translator', None)
            self._stats = TranslationService.initial_stats()
            if translator is None:
                logger.error("Hy-MT2翻译方案未初始化（请检查 hy_mt2_key 配置），跳过翻译并输出空字幕")
                self._AutoSubv3__save_srt(dest_subtitle, [])
                return
            return translator.translate_zh_subtitle(
                source_subtitle,
                dest_subtitle,
                load_srt=self._AutoSubv3__load_srt,
                save_srt=self._AutoSubv3__save_srt,
            )
        self._stats = TranslationService.initial_stats()
